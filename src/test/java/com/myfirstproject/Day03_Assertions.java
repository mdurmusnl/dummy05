    ...
    * Verification = soft assertion = Not hard assertion which means the test case continue to execute even after FAILURE. Eg : if else
     */
     ...
     public void assertions(){
         Assert.assertEquals(5,5);// MRA said its good to have hard assertion, bcs if this fails then fix it then go next
         Assert.assertEquals("ASSERTION OF EXPECTED AND ACTUAL FAILS",5,5); //we dont see the message if the test passes
